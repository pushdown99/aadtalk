...
module.exports = {
  ...
  setup: function (app) {
    ...
    passport.use (new local (function (username, passwd, done) {
      ...
      if(passwd === admin.passwd) { console.log('login-success', username, passwd); return done (null, admin) }
    ...
    app.get  ('/logout', function(req, res) { delete req.session.passport; delete req.session.group; req.logout(); res.redirect('/login'); });  
